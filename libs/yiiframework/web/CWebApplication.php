...
class CWebApplication extends CApplication
{
	...
	public function processRequest()
	{
		if(is_array($this->catchAllRequest) && isset($this->catchAllRequest[0]))
		{
			...
			$route=$this->getUrlManager()->parseUrl($this->getRequest()); //<!-- 获取路由规则
	...
	{
		if(($ca=$this->createController($route))!==null) //<!-- 根据路由创建对应的对象
		{
	...
	public function createController($route,$owner=null)
	{
		if($owner===null)
			$owner=$this;
		if(($route=trim($route,'/'))==='')
			$route=$owner->defaultController; //<!-- 如果没有设置控制器，这里赋值默认的控制器名
		...
		while(($pos=strpos($route,'/'))!==false)
		{
			...
			if(!isset($basePath))  // first segment
			{
				...
				if(($module=$owner->getModule($id))!==null) //<!-- 是否有设计模块，有就返回其对象
